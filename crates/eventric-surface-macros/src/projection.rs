...
impl Projection {
    pub fn dispatch(&self) -> TokenStream {
        ...
        quote! {
            pub trait #dispatch_trait: #(::eventric_surface::projection::Update<#event>)+* {}

            impl #dispatch_trait for #ident {}

            impl ::eventric_surface::projection::Dispatch for #ident {
                fn dispatch(&mut self, event: &::eventric_surface::projection::DispatchEvent) {
                    match event {
        ...
        quote! {
            impl ::eventric_surface::projection::Recognize for #ident {
                fn recognize<C>(
                    ...
                    ::std::option::Option<::eventric_surface::projection::DispatchEvent>,
    ...
    fn select(&self) -> TokenStream {
        ...
        quote! {
            impl ::eventric_surface::projection::Select for #ident {
...
impl ToTokens for RecognizeMatchArm<'_> {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        ...
        tokens.append_all(quote! {
            _ if event.identifier() == <#event as ::eventric_surface::event::Identifier>::identifier()? => {
                std::option::Option::Some(
                    ::eventric_surface::projection::DispatchEvent::from_event::<C, #event>(codec, event)?
                )
            }
        });
    }
        ...
        if tag.is_empty() {
            tokens.append_all(quote! {
                ::eventric_stream::stream::select::Selector::specifiers(
                    [#(<#event as ::eventric_surface::event::Specifier>::specifier()?),*]
                )
            });
        } else {
            tokens.append_all(quote! {
